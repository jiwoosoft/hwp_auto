...
def create_table_tool(
    ...
    if row_count <= 0 or col_count <= 0:
        return build_tool_response(
            ok=False,
            error_code="INVALID_TABLE_SIZE",
            message="row_count and col_count must be positive integers.",
            suggestion="Use values like 1x1 or larger.",
        )
    ...
    placeholder = f"[TABLE {row_count}x{col_count}]"
